...
class BaselSimple(BaselBase):
    ...
    def reset(self):
        current_kmul_index = self._defaultMultiplierIndex if self.defaultMultiplierIndex != None else self.np_random.randint(
    ...
    def _generateProbabilities(self) -> (float, float, float, float):
        ...
        probNoEC, probNB = norm.cdf([reportedValue,  reportedValue * self._kMultipliersListing[self.state[2]] * sqrt(10)], loc=self._normalMean, scale=self._normalStdDev)
